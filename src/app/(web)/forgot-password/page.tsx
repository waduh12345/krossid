...
import { Mail, ArrowRight, ArrowLeft, ShieldCheck, KeyRound, CheckCircle2 } from 'lucide-react';
...
const securitySteps = [
  {
    id: 1,
    title: "Protect your\nAccount Access.",
    quote: "Gunakan autentikasi dua faktor dan jangan pernah membagikan kode OTP Anda kepada siapapun.",
    tag: "Security Tip"
  },
  {
    id: 2,
    title: "Encrypted\nData Recovery.",
    quote: "Proses pemulihan akun kami menggunakan enkripsi end-to-end untuk memastikan data Anda tetap aman.",
    tag: "Privacy First"
  }
];
...
const ForgotPasswordPage = () => {
  ...
  const prevSlide = () => {
    setDirection(-1);
    setIndex((prev) => (prev - 1 + securitySteps.length) % securitySteps.length);
  };
      ...
      <div className="relative z-10 w-full max-w-5xl bg-white/10 backdrop-blur-2xl rounded-[48px] border border-white/20 shadow-2xl flex flex-col md:flex-row p-6 m-4">
        ...
        <div className="flex-1 p-8 lg:p-12 text-white">
          <div className="mb-10">
            ...
            <h1 className="text-4xl font-bold mb-2 tracking-tight">Forgot Password?</h1>
          ...
          {!isSubmitted ? (
            <form onSubmit={handleResetRequest} className="space-y-8">
              <div className="space-y-2">
                <label className="text-xs text-white/50 ml-4 font-bold uppercase tracking-widest">Email Address</label>
              ...
              <button 
                type="submit"
                disabled={isLoading}
                className="w-full py-4 bg-[#DF9B35] hover:bg-[#c78a2e] rounded-full font-black uppercase tracking-[0.2em] shadow-xl transition-all active:scale-[0.98] text-white flex items-center justify-center gap-3"
              >
            ...
            <div className="text-center p-8 bg-white/5 rounded-[32px] border border-white/10 animate-pulse">
               ...
               <p className="text-white font-medium">Link pemulihan telah dikirim!</p>
               <button 
          ...
          <div className="mt-12 text-center">
            <Link href="/signin" className="flex items-center justify-center gap-2 text-sm text-white/70 hover:text-white transition-colors group">
                <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
                Back to <span className="font-bold underline underline-offset-4">Sign In</span>
            </Link>
          </div>
        </div>
        ...
        <div className="flex-1 relative hidden md:block">
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
              <AnimatePresence custom={direction} mode="wait">
                ...
                >
                  ...
                  <div>
                    <h4 className="text-white text-xl font-bold">{securitySteps[index].tag}</h4>
                    <p className="text-[#367CC0] text-sm mt-1 font-bold uppercase tracking-widest">Security verified</p>
                  </div>

                  <div className="flex gap-3">
                    <button onClick={prevSlide} className="w-12 h-10 bg-[#DF9B35] text-white rounded-xl flex items-center justify-center hover:scale-105 transition-all shadow-lg">
                      <ArrowLeft className="w-5 h-5" />
                    </button>
                    <button onClick={nextSlide} className="w-12 h-10 bg-[#367CC0] text-white rounded-xl flex items-center justify-center hover:scale-105 transition-all shadow-lg">
          ...
          >
             <div className="flex items-center gap-2 mb-3 text-[#7ED321]">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest">Account Protection</span>
             </div>
             ...
             <div className="flex items-center -space-x-2">
                {[1,2,3,4].map(i => (
                  <img key={i} className="w-8 h-8 rounded-full border-2 border-white shadow-sm" src={`https://i.pravatar.cc/150?u=sec${i+90}`} alt="user" />
                ))}
