  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-8 space-y-6">
     ...
     <div className="flex items-center gap-6">
       ...
       <div>
         ...
         {selectedImage && <p className="text-[#7ED321] text-xs mt-1">New photo selected</p>}
       </div>
     </div>

     <form onSubmit={handleProfileUpdate} className="space-y-4">
        <div className="group">
          <label className="text-white/40 text-[10px] font-black uppercase ml-2 mb-1 block">Full Name</label>
          <input 
        ...
        <div className="group">
          <label className="text-white/40 text-[10px] font-black uppercase ml-2 mb-1 block">Email Address</label>
          <input 
        ...
        <div className="group">
          <label className="text-white/40 text-[10px] font-black uppercase ml-2 mb-1 block">Phone Number</label>
          <input 
        ...
        <button type="submit" disabled={isUpdatingProfile} className="bg-[#367CC0] text-white px-6 py-2.5 rounded-full text-xs font-bold flex items-center gap-2">
          {isUpdatingProfile ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} Save Changes
        </button>
     </form>
  </motion.div>
);
...
const SecurityView = ({ 
  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-8 space-y-6">
     <div className="flex items-center gap-3"><Lock className="text-[#DF9B35]" size={24} /><h3 className="text-xl font-bold text-white">Change Password</h3></div>
     <form onSubmit={handleSecurityUpdate} className="space-y-4">
        ...
        <button type="submit" disabled={!canSubmitSecurity || isUpdatingProfile} className="bg-[#DF9B35] text-white px-6 py-2.5 rounded-full text-xs font-bold flex items-center gap-2">
          {isUpdatingProfile ? <Loader2 size={14} className="animate-spin" /> : <Lock size={14} />} Update Password
        </button>
     </form>
  </motion.div>
);
  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <h3 className="text-xl font-bold text-white">My Programs</h3>
      </div>
      <span className="text-xs text-white/50">{myProgramsData?.total || 0} Programs</span>
    </div>
    ...
    {isLoadingPrograms ? (
      ...
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[24px] p-12 text-center">
        ...
        <h3 className="text-xl font-black text-white mb-2">No Programs Yet</h3>
        <p className="text-white/40 text-sm mb-6">You haven&apos;t joined any affiliate programs yet.</p>
        <Link 
          href="/programs" 
            ...
            <div className="p-6">
              ...
              <div className="flex items-center gap-4 mt-4">
                ...
                {program.parameter_value && (
                  <div className="flex items-center gap-2 text-xs text-white/50">
                    <span className="text-[#367CC0]">Ref:</span>
...
const AffiliateView = ({ mySalesData, isLoadingSales, userData }: any) => {
  ...
  const handleCopyLink = (programId: number) => {
    ...
    Swal.fire({
      icon: 'success',
      title: 'Link Disalin!',
      text: 'Link referral sudah disalin ke clipboard.',
      timer: 2000,
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-xl font-bold text-white">My Affiliate Programs</h3>
        </div>
        <span className="text-xs text-white/50">{mySalesData?.total || 0} Programs</span>
      </div>
      ...
      {userReferralCode && (
        <div className="bg-gradient-to-r from-[#DF9B35]/20 to-[#7ED321]/20 border border-[#DF9B35]/30 rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">Kode Referral Anda</p>
      ...
      {isLoadingSales ? (
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[24px] p-12 text-center">
          ...
          <h3 className="text-xl font-black text-white mb-2">No Affiliate Programs Yet</h3>
          <p className="text-white/40 text-sm mb-6">You haven&apos;t joined any affiliate programs as a sales agent.</p>
          <Link 
            href="/programs" 
          ...
          {mySalesData.data.map((program: any) => (
            ...
            >
              ...
              <div className="p-6 space-y-4">
                ...
                {/* Referral Link */}
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-white/40 uppercase tracking-widest">Link Referral</p>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">Share:</span>
                  <button 
                ...
                <div className="flex items-center justify-between pt-4 border-t border-white/5 text-xs text-white/30">
                  ...
                  <span>Joined: {new Date(program.created_at).toLocaleDateString('id-ID')}</span>
                </div>
              </div>
            </motion.div>
          ))}
...
const MyAccountLinkedInUI = () => {
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    if (phoneDigits.length < 10) {
      setProfileErrors(prev => ({ ...prev, phone: "Phone number too short" }));
      Swal.fire({
        icon: "error",
        title: "Validation Error",
  ...
  const handleSecurityUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Update Failed",
        text: error?.data?.message || "Failed to update password. Please try again.",
        background: "#1e293b",
        color: "#fff",
      });
    }
  ...
  // Render Logic
  if (isLoadingUser) return <div className="min-h-screen bg-[#0f172a] flex items-center justify-center text-white">Loading...</div>;
