...
const securityInfo = [
  {
    ...
    title: "Stronger Keys,\nBetter Security.",
    quote:
      "Pastikan password Anda terdiri dari minimal 8 karakter dengan kombinasi angka dan simbol unik.",
    tag: "Pro Tip",
  },
  {
    id: 2,
    title: "Instant\nSynchronization.",
    quote:
      "Setelah diperbarui, password baru Anda akan langsung sinkron ke seluruh ekosistem layanan kami.",
    tag: "System Status",
  },
];
...
const ResetPasswordPage = () => {
  ...
  useEffect(() => {
    ...
    if (!email || !otp) {
      Swal.fire({
        ...
        text: "Silakan ulangi proses reset password.",
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Handle Specific Error: OTP Invalid or Expired
      // Pesan error dari backend: "Invalid or expired OTP."
      if (
        ...
        Swal.fire({
          icon: "error",
          title: "OTP Error",
          text: "Kode OTP tidak valid atau sudah kadaluarsa. Silakan masukkan ulang.",
          confirmButtonText: "Input Ulang OTP",
          confirmButtonColor: "#DF9B35",
        }).then(() => {
          // Redirect kembali ke page Verify Reset agar user bisa input OTP yang benar
          // atau melakukan resend OTP
        ...
        Swal.fire({
          icon: "error",
          title: "Update Failed",
          text: error.data?.message || "Gagal memperbarui password.",
        });
      }
    }
  };
        ...
        <div className="flex-1 p-8 lg:p-12 text-white">
          ...
          <form onSubmit={handleResetPassword} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-4 bg-[#DF9B35] hover:bg-[#c78a2e] rounded-full font-black uppercase tracking-[0.2em] shadow-xl transition-all active:scale-[0.98] text-white flex items-center justify-center gap-3 mt-4 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                ...
                {securityInfo.map((_, i) => (
                  <div
                    ...
                    className={`h-1.5 rounded-full transition-all duration-300 ${
                      index === i ? "w-8 bg-[#DF9B35]" : "w-2 bg-white/20"
                    }`}
                  />
                ))}
              </div>
            ...
            <div className="flex items-center -space-x-2">
              {[1, 2, 3, 4].map((i) => (
                <img
                  ...
                  src={`https://i.pravatar.cc/150?u=reset${i + 120}`}
                  alt="user"
                />
              ))}
