...
type ParameterField = {
  name: string;
  type: "alphanumeric" | "numeric" | "currency";
};
...
export default function ProgramDetail() {
  ...
  const shareDropdownMobileRef = useRef<HTMLDivElement>(null);
  const shareDropdownDesktopRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      ...
      const inMobile = shareDropdownMobileRef.current?.contains(target);
      const inDesktop = shareDropdownDesktopRef.current?.contains(target);
  ...
  const handleParameterChange = (
    fieldName: string,
    fieldType: "alphanumeric" | "numeric" | "currency",
    value: string
  ) => {
    let newValue = value;
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    const parameterValueString =
      parameterFields.length > 0
        ? parameterFields
            .map((field) => {
              const rawValue = parameterValues[field.name] || "";

              if (field.type === "currency") {
                return rawValue.replace(/\D/g, ""); // rupiah → integer
              }
  ...
  return (
    <div className="relative min-h-screen text-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-8 space-y-8">
            <motion.div 
                ...
                className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-lg md:rounded-[48px] overflow-hidden shadow-2xl"
            >
              {/* Premium Banner */}
              <div className="h-80 relative overflow-hidden group">
                ...
                <div className="absolute bottom-6 left-8 bg-white/10 backdrop-blur-md px-5 py-2 rounded-lg md:rounded-2xl flex items-center gap-3 border border-white/20">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="flex flex-col gap-3 mt-2 md:hidden">
                        {/* WhatsApp full-width + Share dropdown */}
                        <div className="flex items-center gap-2">
                          {/* WhatsApp - prominent full-width button */}
                          <button
                            type="button"
                            aria-label="Share to WhatsApp"
                            onClick={shareToWhatsApp}
                            disabled={isSharingToWhatsApp}
                            className="flex-1 flex items-center justify-center gap-2.5 px-5 py-3.5 rounded-xl bg-[#25D366]/15 hover:bg-[#25D366]/25 border border-[#25D366]/40 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                          >
                            {isSharingToWhatsApp ? (
                            ...
                            <span className="text-sm font-bold text-[#25D366]">Update Status WA</span>
                          </button>
                          ...
                          <div className="relative" ref={shareDropdownMobileRef}>
                            <button
                              ...
                              className="flex items-center gap-1.5 px-4 py-3.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/20 transition-all"
                            >
                              <Share2 className="w-5 h-5 text-white/70" />
                              <span className="text-sm font-bold text-white/70">Share</span>
                              <ChevronDown className={`w-4 h-4 text-white/50 transition-transform ${showShareDropdown ? 'rotate-180' : ''}`} />
                            </button>
                            {/* Dropdown menu */}
                            <AnimatePresence>
                              {showShareDropdown && (
                                <motion.div
                                  ...
                                  className="absolute right-0 top-full mt-2 w-52 bg-[#1e293b] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50"
                                >
                                  {/* Facebook */}
                                  <button
                                    type="button"
                                  ...
                                  >
                                    ...
                                    <span className="text-sm font-semibold text-white/80">Instagram</span>
                                  </button>
                                  {/* TikTok */}
                                  <button
                                    type="button"
                                  ...
                                  >
                                    ...
                                    <span className="text-sm font-semibold text-white/80">TikTok</span>
                                  </button>
                                  {/* Copy Link */}
                                  <button
                                    type="button"
                                    ...
                                    className="w-full flex items-center gap-3 px-4 py-3 hover:bg-white/5 transition-all border-t border-white/5"
                                  >
                                    <Copy className="w-5 h-5 text-white/60" />
                                    <span className="text-sm font-semibold text-white/80">Copy Link</span>
                                  </button>
                                </motion.div>
                              )}
                            </AnimatePresence>
                      ...
                      <div className="hidden md:flex items-center gap-3 mt-2">
                        {/* Update Status WA - prominent button */}
                        <button
                          type="button"
                          aria-label="Share to WhatsApp"
                          onClick={shareToWhatsApp}
                          disabled={isSharingToWhatsApp}
                          className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#25D366]/10 hover:bg-[#25D366]/20 border border-[#25D366]/30 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                        >
                          {isSharingToWhatsApp ? (
                        ...
                        <div className="relative" ref={shareDropdownDesktopRef}>
                          <button
                            type="button"
                            aria-label="Share to other platforms"
                            onClick={() => setShowShareDropdown(!showShareDropdown)}
                            className="flex items-center gap-1.5 px-3 py-2 rounded-full bg-white/5 hover:bg-white/10 border border-white/20 transition-all"
                          >
                            <Share2 className="w-4 h-4 text-white/70" />
                            <span className="text-xs font-bold text-white/70">Share</span>
                            <ChevronDown className={`w-3.5 h-3.5 text-white/50 transition-transform ${showShareDropdown ? 'rotate-180' : ''}`} />
                          </button>
                          <AnimatePresence>
                            {showShareDropdown && (
                              <motion.div
                                ...
                                exit={{ opacity: 0, y: -8, scale: 0.95 }}
                                transition={{ duration: 0.15 }}
                                className="absolute left-0 top-full mt-2 w-52 bg-[#1e293b] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50"
                              >
                                {/* Facebook */}
                                <button
                                  type="button"
                                ...
                                >
                                  <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none">
                                    <radialGradient id="ig-d" cx="50%" cy="50%" r="80%">
                                      <stop offset="0%" stopColor="#f9ce34"/>
                                      <stop offset="50%" stopColor="#ee2a7b"/>
                                      <stop offset="100%" stopColor="#6228d7"/>
                                    </radialGradient>
                                    <rect x="2" y="2" width="20" height="20" rx="6" fill="url(#ig-d)" />
                                    <circle cx="12" cy="12" r="5" stroke="#fff" strokeWidth="2" fill="none"/>
                                    <circle cx="17" cy="7" r="1.5" fill="#fff"/>
                                  </svg>
                                  <span className="text-sm font-semibold text-white/80">Instagram</span>
                ...
                {(valueDescriptions.length > 0 || benefitDescriptions.length > 0) && (
                  <div className={`grid gap-10 ${valueDescriptions.length > 0 && guideDescriptions.length > 0 ? 'md:grid-cols-2' : 'md:grid-cols-1'}`}>
                    ...
                    {valueDescriptions.length > 0 && (
                      <div className="bg-black/20 p-8 rounded-lg md:rounded-[32px] border border-white/5">
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            {!isSalesRole ? (
              <div 
                className="bg-[#0a0a0a] p-6 md:p-10 rounded-lg md:rounded-[48px] shadow-2xl relative overflow-hidden border border-white/5"
      ...
      <AffiliateRegisterModal 
        ...
        email={programData?.owner_email}
      />

      {/* Register Program Modal - Compact Version */}
      <AnimatePresence>
        {showRegisterModal && (
          <>
            ...
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 pointer-events-none">
              ...
              <div className="relative px-5 pt-4 pb-3 border-b border-white/10">
                <button
                  ...
                  className="absolute top-3 right-3 w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-white/60 hover:text-white hover:bg-white/10 transition-all"
                >
                  <X className="w-4 h-4" />
                </button>
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#367CC0] to-[#7ED321] flex items-center justify-center">
