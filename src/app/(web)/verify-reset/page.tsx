...
const recoveryInfo = [
  {
    ...
    title: "Secure\nAccount Recovery.",
    quote:
      "Verifikasi dua langkah memastikan hanya pemilik akun sah yang dapat melakukan perubahan kredensial keamanan.",
    tag: "Recovery Shield",
  },
  {
    id: 2,
    title: "Protect your\nDigital Identity.",
    quote:
      "Jangan pernah memberikan kode verifikasi ini kepada siapa pun, termasuk pihak yang mengaku dari Kross.id.",
    tag: "Identity Guard",
  },
];
...
const OTPResetPage = () => {
  ...
  // Hooks API (Hanya Resend OTP yang dipanggil di sini)
  ...
  useEffect(() => {
    ...
    if (!email) {
      Swal.fire({
        ...
        text: "Silakan ulangi permintaan reset password.",
  ...
  // --- LOGIC UTAMA: DEFERRED VALIDATION ---
  const handleVerifyReset = async (e: React.FormEvent) => {
    ...
    // Kita simpan OTP di session storage.
    // Validasi ke backend akan dilakukan di page selanjutnya (Reset Password)
    // karena endpoint backend membutuhkan password untuk memvalidasi OTP.
    sessionStorage.setItem("reset_otp", code);

    // Simulasi loading agar UX terasa natural
    setTimeout(() => {
      setIsLoading(false);

      // Langsung redirect ke page input password baru
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Failed",
        text: error.data?.message || "Gagal mengirim ulang OTP.",
      });
    }
  };
        ...
        <div className="flex-1 p-8 lg:p-12 text-white">
          <div className="mb-10">
            ...
            <p className="text-white/70 text-sm italic">
              Enter the security code to authorize your password reset.
            </p>
          </div>
            ...
            <div className="space-y-6">
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    Confirm Identity <ArrowRight className="w-5 h-5" />
                  </>
                )}
              </button>
              ...
              <div className="flex flex-col items-center gap-4">
                <div className="flex items-center gap-2 text-sm font-medium text-white/60">
                  ...
                  <span>
                    Request new code in{" "}
                    <span className="text-[#DF9B35] font-bold">
                ...
                {timer === 0 && (
                  <button
                    ...
                    className="text-[#DF9B35] font-bold underline underline-offset-4 hover:text-white transition-colors text-sm flex items-center gap-2"
                  >
                    {isResendLoading ? (
          ...
          <div className="mt-12 text-center">
            <Link
              ...
              className="text-sm text-white/50 hover:text-white transition-colors flex items-center justify-center gap-2 group"
            >
              <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
              Return to{" "}
              <span className="font-bold underline underline-offset-4 text-white text-xs">
                Reset Request
              </span>
            </Link>
          </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all ${
                    index === 0 ? "bg-[#DF9B35] w-8" : "bg-white/20 w-2"
                  }`}
                />
                <button
                  onClick={() => setIndex(1)}
                  className={`h-1.5 rounded-full transition-all ${
                    index === 1 ? "bg-[#DF9B35] w-8" : "bg-white/20 w-2"
                  }`}
                />
              </div>
            </div>
            ...
            <div className="flex items-center -space-x-2">
              {[1, 2, 3, 4].map((i) => (
                <img
                  ...
                  src={`https://i.pravatar.cc/150?u=resetotp${i + 180}`}
                  alt="user"
                />
              ))}
              <div className="w-8 h-8 rounded-full bg-[#DF9B35] text-white text-[9px] flex items-center justify-center border-2 border-white font-black shadow-sm">
                +2
              </div>
            </div>
          </div>
