...
import { Github, Facebook, ArrowRight, ArrowLeft } from 'lucide-react';
...
const testimonials = [
  ...
  },
  {
    ...
    quote: "Platform ini membantu saya menemukan talenta terbaik di bidang teknologi hanya dalam hitungan hari.",
    name: "Siska Amelia",
      ...
      <div className="relative z-10 w-full max-w-5xl bg-white/10 backdrop-blur-2xl rounded-[48px] border border-white/20 flex flex-col md:flex-row p-6 m-4 shadow-2xl">
        ...
        <div className="flex-1 p-8 lg:p-12 text-white">
          <div className="mb-10">
            ...
            <p className="text-white/70 text-sm italic">Please Enter your Account details</p>
          </div>

          <form className="space-y-6">
            <div className="space-y-2">
              <label className="text-xs text-white/50 ml-4 font-bold uppercase tracking-widest">Email</label>
              <input 
                type="email" 
            ...
            <div className="space-y-2">
              <label className="text-xs text-white/50 ml-4 font-bold uppercase tracking-widest">Password</label>
              <input 
                type="password" 
                placeholder="••••••••"
            ...
            <div className="flex items-center justify-between text-[13px] px-2">
              <label className="flex items-center gap-2 cursor-pointer text-white/70">
                <input type="checkbox" className="rounded bg-black/40 border-white/10 text-[#367CC0]" />
                <span>Keep me logged in</span>
              </label>
              <Link href="/forgot-password" className="text-white/70 hover:text-[#DF9B35] transition-colors underline underline-offset-4 font-medium">Forgot Password</Link>
            </div>

            <button className="w-full py-4 bg-[#DF9B35] hover:bg-[#c78a2e] rounded-full font-black uppercase tracking-[0.2em] shadow-xl transition-all active:scale-[0.98] text-white">
              Sign in
            </button>
          </form>
          ...
          {/* Social Icons */}
            ...
            <p className="text-sm text-gray-300">
              Don't have an account? <Link href="/signup" className="text-white font-bold underline underline-offset-4">Sign Up</Link>
            </p>
            </div>
        </div>
        ...
        <div className="flex-1 relative hidden md:block">
          <div 
            ...
            style={{ 
              maskImage: 'radial-gradient(circle 45px at calc(100% - 10px) 10px, transparent 100%, black 101%)',
              ...
              <AnimatePresence custom={direction} mode="wait">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[#367CC0] text-sm mt-1 font-bold uppercase tracking-widest">{testimonials[index].role}</p>
                  </div>
          ...
          >
             ...
             <div className="flex items-center -space-x-2">
                {[1,2,3,4].map(i => (
                  <img key={i} className="w-8 h-8 rounded-full border-2 border-white shadow-sm" src={`https://i.pravatar.cc/150?u=${i+60}`} alt="user" />
                ))}
