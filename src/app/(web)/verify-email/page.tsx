...
import type { ApiErrorResponse } from "@/lib/error-handle"; // Atau path dimana interface disimpan
...
const verifySteps = [
  {
    ...
    title: "Instant\nAccount Activation.",
    quote:
      "Verifikasi email memastikan Anda mendapatkan akses penuh ke seluruh fitur eksklusif kami secara instan.",
    tag: "Proses Cepat",
  },
  {
    id: 2,
    title: "Double Layer\nProtection.",
    quote:
      "Sistem OTP kami menjamin bahwa hanya Anda yang memiliki kendali penuh atas akun dan aset digital Anda.",
    tag: "Keamanan Tinggi",
  },
];
...
const OTPVerifyPage = () => {
  ...
  // Check Token
  useEffect(() => {
    const token = sessionStorage.getItem("temp_reg_token");
    if (!token) {
      Swal.fire({
        icon: "warning",
        title: "Session Expired",
        text: "Silakan registrasi ulang atau login.",
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      await validateOtp({ otp: code, token }).unwrap();

      // Clear session token setelah sukses
      ...
      Swal.fire({
        ...
        text: error.data?.message || "Kode OTP salah atau kadaluarsa.",
      });
    }
  };
        ...
        <div className="flex-1 p-8 lg:p-12 text-white">
          ...
          <form onSubmit={handleVerify} className="space-y-10">
            ...
            <div className="space-y-6">
              <button
                type="submit"
                disabled={isValidateLoading || otp.join("").length < 6}
          ...
          <div className="mt-12 text-center">
            ...
            >
              ...
              Back to{" "}
              <span className="font-bold underline underline-offset-4 text-white">
                Sign In
              </span>
            </Link>
          </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-all ${
                    index === 0 ? "bg-[#DF9B35] w-8" : "bg-white/20"
                  }`}
                />
                <button
                  onClick={() => setIndex(1)}
                  className={`w-3 h-3 rounded-full transition-all ${
                    index === 1 ? "bg-[#DF9B35] w-8" : "bg-white/20"
                  }`}
                />
              </div>
            </div>
            ...
            <div className="flex items-center -space-x-2">
              {[1, 2, 3, 4].map((i) => (
                <img
                  ...
                  src={`https://i.pravatar.cc/150?u=otp${i + 150}`}
                  alt="user"
                />
              ))}
