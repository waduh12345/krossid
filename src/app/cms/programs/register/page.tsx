...
import { useEffect, useMemo, useState, Suspense } from "react"; // Tambahkan Suspense
...
import { useGetProgramsQuery, useGetProgramsByIdQuery } from "@/services/programs/programs.service";
...
function RegisterTableContent() {
  ...
  // 2. Data Fetching
  const { data: progResp, isLoading: loadingProgs } = useGetProgramsQuery({ page: 1, paginate: 100, search: "", owner_id: isOwner ? session?.user?.id as number : undefined, program_category_id: undefined});
  ...
  const { data: programDetail, isLoading: loadingDetail } = useGetProgramsByIdQuery(
    selectedRecord?.program_id ?? 0, 
    { skip: !selectedRecord }
  );
  ...
  const tableData = useMemo(() => {
    // Perbaikan akses data: pastikan merujuk ke programDetail.data jika response API dibungkus object data
    const parameterString = programDetail?.parameter || programDetail?.parameter;
    ...
    <div className="space-y-6 px-4 py-6">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex w-full flex-wrap items-center gap-2">
            ...
            <Button variant="outline" size="icon" onClick={() => { setQ(""); setSelectedProgramId(null); setPage(1); void refetch(); }}>
        ...
        <div className="rounded-2xl border bg-white shadow-sm overflow-hidden">
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y">
                {isFetching && !items.length ? (
                  <tr><td className="px-4 py-10 text-center text-zinc-500" colSpan={7}>Loading...</td></tr>
                ) : items.length ? (
                  items.map((item) => (
                    <tr key={item.id} className="hover:bg-zinc-50/60 transition-colors">
                      <td className="px-4 py-3">
                        <div className="font-bold text-gray-900">{item.program_name}</div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${item.status === 1 ? 'bg-green-50 text-green-700 ring-green-600/20' : 'bg-red-50 text-red-700 ring-red-600/20'}`}>
                          {item.status === 1 ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="px-4 py-2 text-right">
                        <Button size="sm" variant="destructive" onClick={() => handleDelete(item.id)} disabled={deleting} className="h-8 w-8 p-0">
                  ...
                  <tr><td className="px-4 py-12 text-center text-zinc-500" colSpan={7}>No records found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="flex items-center justify-between border-t bg-zinc-50/30 px-4 py-3 text-xs text-zinc-500">
            <div>Total {total} entries • Page {page} of {lastPage}</div>
            ...
            <div className="mt-4">
              <div className="mb-4 space-y-2">
                <div className="flex gap-2">
                  <span className="text-sm font-medium text-zinc-500 w-24">Name</span>
                  <span className="text-sm text-zinc-900">{selectedRecord?.name}</span>
                </div>
                <div className="flex gap-2">
                  <span className="text-sm font-medium text-zinc-500 w-24">Email</span>
                  <span className="text-sm text-zinc-900">{selectedRecord?.email}</span>
                </div>
                <div className="flex gap-2 cols-span-2">
                  <span className="text-sm font-semibold text-zinc-900">{selectedRecord?.program_name}</span>
                </div>
              </div>
              ...
              <div className="rounded-lg border overflow-hidden">
                <table className="min-w-full text-sm divide-y">
                  <thead className="bg-zinc-50">
                    <tr>
                      <th className="px-4 py-2.5 text-left font-semibold text-zinc-700">Parameter</th>
                      <th className="px-4 py-2.5 text-left font-semibold text-zinc-700">Value</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y bg-white">
                    {loadingDetail ? (
                      <tr><td colSpan={2} className="px-4 py-8 text-center text-zinc-500 italic">Memuat detail parameter...</td></tr>
                    ) : tableData.length > 0 ? (
                      tableData.map((row, idx) => (
                        <tr key={idx} className="hover:bg-zinc-50/50">
                          <td className="px-4 py-2.5 font-semibold text-zinc-600 bg-zinc-50/30 w-1/3">{row.label}</td>
                          <td className="px-4 py-2.5 text-zinc-900">{row.value}</td>
                        </tr>
                      ))
                    ) : (
                      <tr><td colSpan={2} className="px-4 py-8 text-center text-zinc-500">Tidak ada data tambahan.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
