...
export default function SuperadminDashboard() {
  ...
  // 3. Cek apakah role user adalah 'owner'
  // Kita cek array roles, jika ada salah satu yang name === 'owner'
  const isOwner = useMemo(() => {
    // Kita melakukan casting (as) agar TypeScript tahu bahwa user memiliki array roles
    const user = session?.user as { roles?: UserRole[] } | undefined;

    // Ambil roles, default ke array kosong jika undefined
    const roles = user?.roles || [];

    // Di sini 'r' otomatis dikenali sebagai UserRole, jadi tidak butuh 'any'
    return roles.some((r) => r.name === "owner");
  }, [session]);

  // --- INTEGRASI SERVICE ---
  // Jika owner, kita tidak perlu fetch total owners (opsional, untuk hemat bandwidth)
  const { data: totalOwners = 0, refetch: refetchOwners } =
    useGetTotalOwnersQuery(undefined, { skip: isOwner });

  const { data: totalPrograms = 0, refetch: refetchPrograms } =
    useGetTotalProgramsQuery();
  const { data: totalRegistrations = 0, refetch: refetchRegis } =
    useGetTotalProgramRegistrationsQuery();
  const { data: totalViews = 0, refetch: refetchViews } =
    useGetTotalProgramViewsQuery();

  const handleRefresh = () => {
    if (!isOwner) refetchOwners();
    refetchPrograms();
    refetchRegis();
    refetchViews();
  };
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "RecentActivity");
    XLSX.writeFile(wb, "Network_Activity_Log.xlsx");
  };

  // 4. Mapping Data & Filtering
  const stats = useMemo(() => {
    // Definisi semua stats
    const allStats = [
      {
        label: "Total Owners",
        value: totalOwners,
    ...
    // Filter: Jika isOwner true, buang item yang labelnya "Total Owners"
    if (isOwner) {
      return allStats.filter((item) => item.label !== "Total Owners");
    }

    return allStats;
  }, [totalOwners, totalPrograms, totalRegistrations, totalViews, isOwner]);
  ...
  return (
    <div className="relative flex min-h-screen flex-col bg-[#FAFBFC] dark:bg-neutral-950">
      ...
      <main className="mx-auto w-full max-w-[1600px] px-4 py-6 md:px-8 md:py-10">
        ...
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="bg-[#4A90E2] text-white text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-widest">
                {/* Tampilkan role dinamis atau default */}
                {isOwner ? "Owner" : "Superadmin"}
              </span>
              <span className="text-[10px] text-[#8E8E8E] font-bold uppercase tracking-widest">
                Network Status: <span className="text-[#7ED321]">Stable</span>
              </span>
            </div>
            ...
            <p className="text-sm text-[#8E8E8E]">
              Global performance and traffic harvester monitor.
            </p>
          </div>

          <button
            onClick={handleRefresh}
            className="flex items-center gap-2 rounded-xl bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 px-4 py-2.5 text-sm font-bold shadow-sm hover:bg-gray-50 transition-all cursor-pointer active:scale-95"
          >
        ...
        {/* KPI CARDS */}
        {/* Grid disesuaikan: Jika owner (3 items), grid menyesuaikan */}
        <section
          className={`grid grid-cols-1 gap-4 sm:grid-cols-2 ${
            isOwner ? "lg:grid-cols-3" : "lg:grid-cols-4"
          } md:gap-6 mb-8`}
        >
          {stats.map((item) => (
            <div
              key={item.label}
              className="group relative overflow-hidden rounded-[2rem] border border-white bg-white/70 p-6 shadow-sm backdrop-blur-xl transition-all hover:-translate-y-1 hover:shadow-xl dark:bg-neutral-900/60 dark:border-neutral-800"
            >
              ...
              <div>
                <p className="text-xs font-bold text-[#8E8E8E] uppercase tracking-widest mb-1">
                  {item.label}
                </p>
            ...
            <div className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-xs text-[#8E8E8E]">
                  Live Traffic (Green) vs Fraud Attempts (Grey)
                </p>
              </div>
          ...
          <div className="bg-white dark:bg-neutral-900 p-6 md:p-8 rounded-[2.5rem] border border-gray-100 dark:border-neutral-800 shadow-sm">
            <div className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-xs text-[#8E8E8E]">
                  Flat-Based (Blue) vs Dynamic-Based (Orange)
                </p>
              </div>
          ...
          <div className="p-8 border-b border-gray-50 flex justify-between items-center">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-2 text-xs font-bold text-white bg-[#7ED321] px-4 py-2 rounded-lg hover:bg-[#6ec210] transition-colors"
              >
                <Download size={14} />
                Export Excel
              </button>
              <button className="text-xs font-bold text-[#4A90E2] hover:underline">
                View All Network
              </button>
            </div>
          </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-50 text-sm">
                {TABLE_DATA.map((row, i) => (
                  ...
                  >
                    <td className="px-8 py-5 font-bold text-[#1A1A1A] dark:text-white uppercase tracking-tighter">
                      {row.name}
                    </td>
                    <td className="px-8 py-5 text-[#8E8E8E] font-medium italic">
                      @{row.domain}
                    </td>
                    <td className="px-8 py-5 text-[#4A90E2] font-bold">
                      {row.action}
                    </td>
